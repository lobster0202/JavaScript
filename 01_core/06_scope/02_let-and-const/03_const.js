/*  3. const 
    ...
    변수 호이스팅이 발생하지 않는 것 처럼 보인다.
    아래에서는 let 키워드와의 차이점을 중심으로 살펴보자      
    
    < 상수 : 재할당이 금지된 변수 >
    const 키워드로 선언된 변수에 원시 값을 할당하면
    원시 값은 변경 할 수 없는 값이고
    const 키워드에 의해 재할당이 금지되므로
    값을 변경 할 방법은 없다.
    
    일반적으로 상수의 이름은 대문자로 선언해 
    상수임을 명확히 나타내며
    여러 단어로 이루어진 경우 언더스코어(_)로 구분하는
...
/*  < 전역 변수의 문제점 >
    ...
    3.  "스코프 체인 상에서 종점에 존재" 하므로 
        전역 변수의 검색 속도가 가장 느리다.

    4.  "네임 스페이스가 오염"된다.

        자바 스크립트는 파일이 분리 되어 있어도 
        하나의 전역 스코프를 공유 하므로
        예상치 못한 결과를 가져올 수 있다.
