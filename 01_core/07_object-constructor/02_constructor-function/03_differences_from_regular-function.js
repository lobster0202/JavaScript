...
// 일반 함수로서 호출된 Teacher는 반환문이 없으므로 undefined 
console.log(teacher); 

// 일반 함수로서 호출된 Teacher 내의 this는 전역 객체를 가리킨다.
console.log(hobby);

/*  new.target 
    (생성자 함수가 new 연산자 없이 
    호출되는 것을 방지하기 위해 ES6에서 지원)
    new 연산자와 함께 생성자 함수로써 호출 시 
    new.target은 함수 자신을 가리킨다.

    new 연산자 없이 일반 함수로 호출 시 
    함수 내부의 new.target은 undefined이다. */

function Dog(name, skill) {

    if(!new.target){            // new.target = undefined = falsy
